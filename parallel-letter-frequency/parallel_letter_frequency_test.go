...
func TestConcurrentFrequency(t *testing.T) {
	seq := Frequency(euro + dutch + us + euro + dutch + us + euro + dutch)
	con := ConcurrentFrequency([]string{euro, dutch, us, euro, dutch, us, euro, dutch})
...
func BenchmarkSequentialFrequency(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Frequency(euro + dutch + us + euro + dutch + us + euro + dutch)
	}
}

func BenchmarkConcurrentFrequency(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ConcurrentFrequency([]string{euro, dutch, us, euro, dutch, us, euro, dutch})
	}
}
